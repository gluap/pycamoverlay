...
class arrayValueDisplay(object):
    ...
    def draw(self, frame, fast=False):
        ...
        if not fast:
            return self.drawValue(self.array[frame])
        elif fast:
            return self.drawValueFast(self.array[frame])
...
class heartRateDisplay(arrayValueDisplay):
    def __init__(self, speedF, size=[200, 60]):
        ...
        super(heartRateDisplay, self).__init__(speedF, size, symbolBefore=u" heartrate ", symbolAfter="")


class sensorVelocityDisplay(arrayValueDisplay):
    def __init__(self, speedF, size=[200, 60]):
        self.caption = "SPEED"
        super(sensorVelocityDisplay, self).__init__(speedF * 60 * 2.136 / 1000., size, symbolBefore=u"",
                                                    symbolAfter=" km/h")
...
class temperatureDisplay(arrayValueDisplay):
    def __init__(self, speedF, size=[200, 60]):
        ...
        super(temperatureDisplay, self).__init__(speedF, size, symbolBefore=u"", symbolAfter=u"deg")
