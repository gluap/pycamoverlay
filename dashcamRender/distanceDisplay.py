...
class distanceDisplay(object):
    ...
    def fastDraw(self,frame):
        ...
        draw.text((10, 10), "%0.1f(%0.1f)" % (self.distances.getDistance(frame),self.distances.getSigma(frame)), (0,0,0,255), font=font)
        return temp
...
class scaleBar(object):
    ...
    def draw(self,value,sigma=0.):
        ...
        draw.line((self.width-width,0,self.width-width,self.height/2),fill=(255,255,255,200),width=int(widthSigma)+1)
        draw.line((self.width-self.badWidth,0,self.width-self.badWidth,self.height/2+16),fill=(0,0,0,200),width=3)
